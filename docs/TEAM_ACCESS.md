...
Travel catalog keys are `vehicles.view`, `vehicles.manage`, and `vehicles.delete` (vehicles registry and trip log book). Smart AI import (fleet/GPS exports) requires `vehicles.manage` and a team AI provider. Confirmed imports are tracked as batches; undo (remove all trips from that import) and multi-select bulk delete on the log book require `vehicles.delete`. Licence disc expiry reminders email team members who have `vehicles.view` (and have not opted out under Profile preferences).
