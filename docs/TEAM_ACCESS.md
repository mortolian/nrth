...
Money Out catalog keys include `expenses.*` and `suppliers.*`. Recurring expense templates use the same `expenses.view` / `expenses.manage` / `expenses.delete` keys as one-off expenses.
