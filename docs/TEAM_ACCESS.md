...
Money In catalog keys include `invoices.*`, `estimates.*`, `clients.*`, and `items.*` (products/services catalog). Recurring invoices use the same `invoices.view` / `invoices.manage` / `invoices.delete` keys as one-off invoices. Invoice **note templates** live under **Settings → Business → Note templates** (`settings.business`). Item unit labels are under Settings → Business → Units.

Money Out catalog keys include `expenses.*` and `suppliers.*`. Recurring expense templates use the same `expenses.view` / `expenses.manage` / `expenses.delete` keys as one-off expenses. Marking an expense as not requiring a receipt (and the list-row toggle) requires `expenses.manage`.

Banking catalog keys are `banking.view` and `banking.manage`. **View** covers imported transactions (including matching status) and accounts. **Manage** covers statement import (one or more CSV/OFX files per upload; each file is previewed and confirmed in order), account setup, matching imported lines to posted payments/expenses/journal entries, split allocations, excluding personal/out-of-scope lines, resetting a line to unreviewed, undoing a confirmed import (transactions are removed; the file is kept for re-import), and permanently deleting undone or abandoned statement files from import history. Owners and accountants with both `banking.manage` and `expenses.manage` can also create an expense from an unmatched or partially matched debit on Banking → Transactions; saving posts the expense and allocates it to that bank line. Not every imported line has to be matched — mixed personal/business accounts can leave personal activity excluded or unreviewed. Matching happens on the Banking → Transactions page.
...
Wealth catalog keys are `wealth.view` and `wealth.manage`. **Travel**, **Planning**, and **Wealth** are optional modules (off by default for new businesses under Settings → Features). Existing businesses are opted in for Travel / Planning by migration when upgrading. Disabling a module hides nav and returns 403 without deleting data. Travel covers vehicles and trip log book; Planning covers budgets; Wealth covers portfolios, assets, valuation snapshots, cash-flow transactions, contribution allowances, and derived investment movement.
